...
export function applyFilter({ inputData, comparator, filterName }: ApplyFilterProps): Facility[] {
  ...
  if (filterName) {
    ...
    filteredData = filteredData.filter(
      (facility) =>
        ...
        (facility.city ?? '').toLowerCase().includes(keyword) ||
        (facility.country ?? '').toLowerCase().includes(keyword)
    );
  }
